...
function BodyReader(options) {
    ...
    function convertToPixels(rawValue, unitType) {
        if (unitType === "EMU") {
            // EMUs: Convert using the EMU-to-pixel ratio
            return rawValue * (96 / 914400);
        } else if (unitType === "DXA") {
            // dxa: Convert from twentieths of a point to pixels
            return (rawValue / 12) * (96 / 72);
        } else {
      ...
      function parseImageProperties(element) {
        let floating = null;
        let wrappingStyle = null;
        let alignment = { horizontal: null, vertical: null };
        let positionOffset = { horizontal: null, vertical: null };
        ...
        if (element.name === "wp:inline") {
            ...
            const wrapElement = element.children.find((child) =>
                child.name.startsWith("wp:wrap")
            );
            wrappingStyle = wrapElement ? wrapElement.name.replace("wp:", "") : null;
        }
    
        // Detect alignment
        const positionH = element.children.find((child) => child.name === "wp:positionH");
        const positionV = element.children.find((child) => child.name === "wp:positionV");
    
        if (positionH) {
            alignment.horizontal = positionH.attributes.relativeFrom;
            const posOffsetH = positionH.children.find((child) => child.name === "wp:posOffset");
            positionOffset.horizontal = posOffsetH ? parseInt(posOffsetH.children[0].value, 10) : null;
        }
    
        if (positionV) {
            alignment.vertical = positionV.attributes.relativeFrom;
            const posOffsetV = positionV.children.find((child) => child.name === "wp:posOffset");
            positionOffset.vertical = posOffsetV ? parseInt(posOffsetV.children[0].value, 10) : null;
        }
    
        return { floating, wrappingStyle, alignment, positionOffset };
    }  
    ...
    function readBlip(element, blip) {
      ...
      // Parse floating, wrapping style, alignment, and position offset
      const { floating, wrappingStyle, alignment, positionOffset } = parseImageProperties(element);

      if (blipImageFile === null) {
          return emptyResultWithMessages([warning("Could not find image file for a:blip element")]);
      } else {
          return readImage(blipImageFile, altText, width, height, floating, wrappingStyle, alignment, positionOffset);
      }
    }
    ...
    function readImage(imageFile, altText, width, height, floating, wrappingStyle, alignment, positionOffset) {
