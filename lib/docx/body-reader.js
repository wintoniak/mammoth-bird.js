...
function BodyReader(options) {
    ...
    function readTable(element) {
      // Combine firstOrEmpty("w:tblPr") and firstOrEmpty("w:tblBorder")
        var bordersResult = readTableBorders(element.firstOrEmpty("w:tblBorders"));
        var propertiesResult = readTableProperties(element.firstOrEmpty("w:tblPr"));
        var combinedPropertiesResult = ReadResult.map(bordersResult, propertiesResult, function(borders, properties) {
            return _.extend({}, properties, borders);
        });
        console.log('combinedPropertiesResult', combinedPropertiesResult);
        return readXmlElements(element.children)
            .flatMap(calculateRowSpans)
            .flatMap(function(children) {
                return combinedPropertiesResult.map(function(properties) {
    ...
    function readTableBorders(element) {
      console.log(element);
        var optionsElement = element.first("w:tblBorders");
        console.log(optionsElement);
        var isBordered = false;
        if (optionsElement && optionsElement.children && optionsElement.children.length > 0) {
            isBordered = true;
        }
    ...
    function readTableCell(element) {
        return readXmlElements(element.children).map(function(children) {
            ...
            var width = properties.firstOrEmpty("w:tcW").attributes["w:w"] / 10;
