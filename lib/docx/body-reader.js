...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:ins": function(element) {
            ...
            if (element.attributes["w:author"]) {
                attributes = Object.assign({}, attributes, {authorName: element.attributes["w:author"]});
            }
            if (element.attributes["w:date"]) {
                attributes = Object.assign({}, attributes, {date: element.attributes["w:date"]});
            }
            if (element.attributes["w:id"]) {
                attributes = Object.assign({}, attributes, {changeId: element.attributes["w:id"]});
            }
            if (element.children && element.children.length > 0) {
                ...
                if (propertiesHolder) {
                    ...
                    if (attributesHolder) {
                        ...
                        attributes = Object.assign({}, attributes, childAttributes);
                    }
                }
            }
            return ReadResult.map(
                ...
                function(properties, children) {
                  console.log('properties', properties);
                    ...
                    console.log('insProperties', insProperties);
        ...
        "w:del": function(element) {
            ...
            if (element.children && element.children.length > 0) {
                var propertiesHolder = element.children[0].first("w:rPr");
                if (propertiesHolder) {
                    var attributesHolder = propertiesHolder.firstOrEmpty("w:rPrChange");
                    if (attributesHolder) {
                        var childAttributes = {
                            authorName: attributesHolder.attributes["w:author"],
                            date: attributesHolder.attributes["w:date"],
    ...
    function readTableBorders(element) {
        ...
        if (element.attributes["w:val"] !== "nil" && element.attributes["w:val"] !== "none") {
            isBordered = true;
        }
    ...
    function readBlip(element, blip) {
        ...
        var dimensionsHolder = element.firstOrEmpty("wp:extent").attributes;
        var widthDxa = dimensionsHolder ? dimensionsHolder.cx : null;
        var heightDxa = dimensionsHolder ? dimensionsHolder.cy : null;
        // Convert dxa to pt, then use 20/12 px/pt to convert to px
        var width = widthDxa ? widthDxa / 12 : null;
        var height = heightDxa ? heightDxa / 12 : null;
        if (blipImageFile === null) {
            return emptyResultWithMessages([warning("Could not find image file for a:blip element")]);
        } else {
            return readImage(blipImageFile, altText, width, height);
        }
    }
    ...
    function readImage(imageFile, altText, width, height) {
