...
function BodyReader(options) {
    ...
    function parseImageProperties(element) {
        ...
        if (element.name === "wp:inline") {
            ...
            var wrapElement = element.children.find(function (child) {
                return child.name.startsWith("wp:wrap");
            });
            wrappingStyle = wrapElement ? wrapElement.name.replace("wp:", "") : null;
        }
    
        // Detect alignment
        var positionH = element.children.find(function (child) {
            return child.name === "wp:positionH";
        });
        var positionV = element.children.find(function (child) {
            return child.name === "wp:positionV";
        });
    
        if (positionH) {
            alignmentH = positionH.attributes.relativeFrom;
            var posOffsetH = positionH.children.find(function (child) {
                return child.name === "wp:posOffset";
            });
        ...
        if (positionV) {
            ...
            var posOffsetV = positionV.children.find(function (child) {
                return child.name === "wp:posOffset";
            });
            positionOffsetV = posOffsetV ? parseInt(posOffsetV.children[0].value, 10) : null;
        }
    
        return { floating, wrappingStyle, alignmentH, alignmentV, positionOffsetH, positionOffsetV };
    }
    ...
    function readBlip(element, blip) {
      ...
      if (blipImageFile === null) {
          ...
          return readImage(blipImageFile, altText, width, height, formatOptions.floating, formatOptions.wrappingStyle, formatOptions.alignmentH, formatOptions.positionOffsetH, formatOptions.alignmentV, formatOptions.positionOffsetV);
      }
    }
    ...
    function readImage(imageFile, altText, width, height, floating, wrappingStyle, alignmentH, positionOffsetH, alignmentV, positionOffsetV) {
