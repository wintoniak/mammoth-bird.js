...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:ins": function(element) {
            ...
            console.log(element)
            console.log(element.firstOrEmpty("w:rPr"))
            console.log(element.firstOrEmpty("w:rprChange"))
            console.log(element.children[0].first("w:rPr").firstOrEmpty("w:rPrChange"))
            return ReadResult.map(
                readRunProperties(element.firstOrEmpty("w:rPr")),
                readXmlElements(element.children),
                function(properties, children) {
                    children = [new documents.Ins(children, properties)];
                    return new documents.Run(children, properties);
                }
            );
        },
        "w:del": function(element) {
            ...
            return ReadResult.map(
                ...
                readXmlElements(element.children),
                function(properties, children) {
                    children = [new documents.Del(children, properties)];
                    return new documents.Run(children, properties);
                }
            );
        },
