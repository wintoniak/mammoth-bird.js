...
function BodyReader(options) {
    ...
    function readBlip(element, blip) {
      ...
      // Extract floating and alignment properties
      var floating = element.first("wp:wrapSquare")
      ? "wrapSquare"
      : element.first("wp:wrapNone")
      ? "wrapNone"
      : null;

      var alignment = element.first("wp:positionH wp:align")
      ? element.first("wp:positionH wp:align").text
      : null;

      /* var positionH = element.first("wp:positionH")
        ? element.first("wp:positionH").attributes.relativeFrom
        : null;

      var positionV = element.first("wp:positionV")
        ? element.first("wp:positionV").attributes.relativeFrom
        : null; */

      if (blipImageFile === null) {
          return emptyResultWithMessages([warning("Could not find image file for a:blip element")]);
      } else {
          return readImage(blipImageFile, altText, width, height, floating, alignment);
      }
    }
