...
function BodyReader(options) {
    ...
    function readBlip(element, blip) {
        ...
        var widthHolder = element.firstOrEmpty("wp:extent").attributes;
        var width = widthHolder ? widthHolder.cx : null;
        if (blipImageFile === null) {
            return emptyResultWithMessages([warning("Could not find image file for a:blip element")]);
        } else {
            return readImage(blipImageFile, altText, width);
        }
    }
    ...
    function readImage(imageFile, altText, width) {
