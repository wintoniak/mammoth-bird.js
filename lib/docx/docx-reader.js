...
function read(docxFile, input) {
  ...
  return promises.props({
      ...
      return readStylesFromZipFile(docxFile, result.partPaths.styles).then(function(styles) {
          result.styles = styles;
          return result;
      });
  }).then(function(result) {
      return readNumberingFromZipFile(docxFile, result.partPaths.numbering, result.styles).then(function(numbering) {
          result.numbering = numbering;
          return result;
      });
  }).then(function(result) {
      return Promise.all([
          readXmlFileWithBody(result.partPaths.footnotes, result, function(bodyReader, xml) {
              return xml ? notesReader.createFootnotesReader(bodyReader)(xml) : new Result([]);
          }),
          readXmlFileWithBody(result.partPaths.endnotes, result, function(bodyReader, xml) {
              return xml ? notesReader.createEndnotesReader(bodyReader)(xml) : new Result([]);
          }),
          readXmlFileWithBody(result.partPaths.comments, result, function(bodyReader, xml) {
              return xml ? commentsReader.createCommentsReader(bodyReader)(xml) : new Result([]);
          })
      ]).then(function([footnotes, endnotes, comments]) {
          result.footnotes = footnotes;
          result.endnotes = endnotes;
          result.comments = comments;

          result.notes = result.footnotes.flatMap(function(footnotes) {
