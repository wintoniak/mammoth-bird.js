...
function readCustomStyles(docxFile) {
  const customStyles = [];
  return docxFile.read("word/styles.xml").then(function(stylesXml) {
      const parser = new DOMParser();
      const xmlDoc = parser.parseFromString(stylesXml, "text/xml");

      xmlDoc.querySelectorAll("w|style").forEach(styleElement => {
          const customStyle = styleElement.getAttribute("w:customStyle");
          if (customStyle === "1") {
              const styleId = styleElement.getAttribute("w:styleId");
              const styleName = styleElement.querySelector("w|name").getAttribute("w:val");
              customStyles.push({
                  id: styleId,
                  name: styleName
              });
