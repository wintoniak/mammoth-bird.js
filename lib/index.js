...
function convertToHtml(input, options) {
    const convertRes = convert(input, options);
        ...
        .then(function(docxFile) {
            return docxReader.read(docxFile, input)
                .then(function(documentResult) {
                    const transformedDocument = documentResult.map(options.transformDocument);
                    // Attach customStyles to the transformed document result
                    return {
                        document: transformedDocument,
                        customStyles: documentResult.customStyles
                    };
                })
                .then(function(result) {
                    // Pass the result with both the document and customStyles to convertDocumentToHtml
                    return convertDocumentToHtml(result.document, options).then(function(htmlResult) {
                        // Ensure the customStyles are included in the final return
                        return {
                            html: htmlResult,
        ...
        return styleMapResult.flatMapThen(function(styleMap) {
            return documentConverter.convertToHtml(document)
                .then(function(htmlResult) {
                    return {
                        value: htmlResult.html,
                        customStyles: htmlResult.customStyles,
                    };
                });
        });
    });
