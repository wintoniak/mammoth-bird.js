    ...
    return unzip.openZip(input)
        ...
        .then(function(docxFile) {
            return docxReader.read(docxFile, input)
                ...
                .then(function(documentResult) {
                  console.log(documentResult.styles)
                    // Read styles XML to get custom styles
                    return docxFile.read("word/styles.xml")
                        .then(function(stylesXml) {
                            const stylesXmlString = uint8ArrayToString(stylesXml);
                            // Parse the XML using readString from xml module
                            return xmlReader(stylesXmlString).then(function(stylesDocument) {
                                console.log(1);
                                var styles = readStylesXml(stylesDocument);
                                console.log(styles.getCustomStyles());
                                options.customStyles = styles.getCustomStyles();
