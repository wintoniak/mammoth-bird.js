        ...
        .then(function(docxFile) {
            return docxReader.read(docxFile, input)
                .then(function(result) {
                    // `result` now contains both `document` and `styles`
                    const transformedDocument = result.document.map(options.transformDocument);
                    
                    // Keep `document` and `styles` together in the result
                    return {
                ...
                .then(function(result) {
                    return convertDocumentToHtml(result.document, options)
                        .then(function(htmlResult) {
                            // Include styles in the final result
                            return {
                                value: htmlResult.value,
                                customStyles: result.styles.getCustomStyles()
                            };
                        });
                });
        });
